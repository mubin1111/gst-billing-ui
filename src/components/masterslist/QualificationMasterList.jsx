...
export default function QualificationList() {
  ...
  const [selectedRows, setSelectedRows] = useState([]); // store QualificationCode
  ...
  const onDeleteRow = (code) => {
    pushToast(
      "error",
      `Simulating DELETE for Qualification Code ${code}. Add confirmation/modal in real app.`,
      5000
    );
  };

  // sample qualification data (replace with API data)
  ...
  const exportExcel = (selectedOnly = false) => {
    const src =
      selectedOnly && selectedRows.length > 0
        ? qualifications.filter((c) =>
            selectedRows.includes(c.QualificationCode)
          )
        : qualifications;

    if (selectedOnly && selectedRows.length === 0) {
      pushToast("error", "No qualifications selected for export.");
      return;
    }
    if (!src.length) {
    ...
    XLSX.writeFile(workbook, "QualificationList.xlsx");
    pushToast("success", "Qualification List exported to Excel.");
  };
  ...
  const exportPDF = (selectedOnly = false) => {
    ...
    doc.setFontSize(16);
    doc.setTextColor(30);
    doc.text("Qualification List", 14, 15);

    const head = [
      ["Qualification Code", "Qualification Name", "Type", "Description"],
    ];
    ...
    doc.save("QualificationList.pdf");
    pushToast("success", "Qualification List exported to PDF.");
  };
  ...
  const printTable = (selectedOnly = false) => {
    ...
    if (selectedOnly && selectedRows.length === 0) {
      pushToast("error", "No qualifications selected for print.");
      return;
    }
    if (!src.length) {
    ...
    const rows = src
      .map(
        (e) => `
      <tr>
        <td>${e.QualificationCode}</td>
        <td>${e.QualificationName}</td>
        <td>${e.QualificationType}</td>
        <td>${e.Description}</td>
      </tr>
    `
      )
    ...
    const html = `
      <html><head><title>Qualification List Print</title>
      <style>
      ...
      <h3>Qualification List (${selectedOnly ? 'Selected Only' : 'All Qualifications'})</h3>
      <table>
        <thead>
          <tr>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-5 p-3 rounded-lg bg-slate-50 border border-slate-200">
        <div className="flex items-center gap-4 text-sm text-slate-600">
          <span className="font-semibold">
            {selectedRows.length} qualification
              ...
              <td className="px-4 py-3">
                <div className="flex items-center justify-center gap-1">
                  ...
                  <button
                    onClick={() =>
                      navigate(
                        `/qualification-master?code=${e.QualificationCode}`
                      )
                    }
                  ...
                  <button
                    onClick={() => onDeleteRow(e.QualificationCode)}
